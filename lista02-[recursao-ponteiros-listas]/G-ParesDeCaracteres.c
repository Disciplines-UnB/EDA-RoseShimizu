...
int ContaPares(char* string, int index){
    if(string[index] == '\0' || string[index + 1] == '\0' || string[index + 2] == '\0'){return 0;}
    if(string[index] == string[index + 2]){return 1 + ContaPares(string, index+1);}
    else{return ContaPares(string, index + 1);}
};

int main(){
    char string[210];
    scanf("%s", string);
    int resultado = ContaPares(string, 0);
    printf("%d\n", resultado);
    return 0;
};
