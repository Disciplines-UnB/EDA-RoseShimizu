...
typedef struct lista{int dado; struct lista *prox;}lista;
lista *cria_lista(int *valores_xi, int p){
    lista *cabeca = malloc(sizeof(lista)), *temp = cabeca;
    for (int i = 0; i < p; i++)
    {
        lista *nova = malloc(sizeof(lista));
        temp->prox = nova; temp = temp->prox; temp->dado = valores_xi[i];
    }
    return cabeca;
}

void remove_depois(lista *pont){
    if (pont == NULL){return;}
    else if (pont->prox == NULL){pont->prox = NULL;}
    else{lista *temp = pont->prox; pont->prox = temp->prox;}
}

void elimina(lista *cabeca, int ordem, int *valores_ai){
    lista *temp = cabeca;
    for (int i = 0; temp->prox != NULL; i++){
        if (valores_ai[i] != ordem)remove_depois(temp);
        else temp = temp->prox;
    }
}
...
int main()
{
    ...
    int p, r, n, j, valores_xi[101], valores_ai[101], teste = 1;
    while (1){
        scanf("%d %d", &p, &r);
        if (p && r){
            for (int i = 0; i < p; i++)scanf("%d", &valores_xi[i]);
            lista = cria_lista(valores_xi, p);
            for (int i = 0; i < r; i++){
                scanf("%d %d", &n, &j);
                for (int k = 0; k < n; k++)scanf("%d", &valores_ai[k]);
